...
int TURN_TICKS_L = 620;//689       //change this left encoder ticks value to calibrate left turn 
...
//Grids           1  untouched----->>>   2     3     4     5     6     7     8    9  
int TICKS[9] = {560, 1075, 1672, 2290, 2890, 3495, 4095, 4685, 5295};  // for movement of each grid
const int LEFTTICK[14] = {20, 25, 30, 35, 40, 233, 50, 55, 489, 65, 70, 75, 80, 85};//adjust LEFTICK[5] for 45 Degree
const int RIGHTTICK[14] = {20, 25, 30, 35, 40, 223, 50, 55, 450, 65, 70, 75, 80, 85};//adjust LEFTICK[5] for 45 Degree
  ...
  while (tick_R <= distance || tick_L+30 <= distance) {
    if (distancee == 10)
    {
      if (myPID.Compute()) {
        ...
        md.setSpeeds(currentSpeed+0.15 * speed_O, currentSpeed + speed_O);
      }
    }
...
void turnRight(int angle) {
  ...
  if (angle >= 90) {
    for (i=0; i<angle; i=i+90) {
      initializeTick();
      initializeMotor_Start();
      while (tick_R < (TURN_TICKS_R-10) || tick_L < (TURN_TICKS_R-10)) { // turn left and right -115 when turn 90 degree to right
        if (myPID.Compute())
          md.setSpeeds((currentSpeed + speed_O), -(currentSpeed - speed_O));
      }
    }
